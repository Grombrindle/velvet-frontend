...
function CheckoutPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const result = res?.result || res;
      const order = result?.order || null;
      const step = result?.next_step || null;
  ...
  }, [isCartLoading, items, router, localePrefix, isAuthenticated, checkoutState, orderResult, nextStep]);

  if (!hydrated) return null;

  if (
    !isAuthenticated ||
    (!isCartLoading && items.length === 0 && !orderResult && checkoutState === "idle")
  ) {
    return null;
  }
      ...
      <div className="min-h-screen pt-8 pb-16">
        <div className="container1 mx-auto px-2 max-w-lg">
          <NextStepHandler
            nextStep={nextStep}
            orderId={orderResult.id}
            paymentFields={paymentFields} // 👈 Fixed: Passing paymentFields prop so NextStepHandler gets identifier/phone
            onComplete={handleCheckoutComplete}
          />
        </div>
      </div>
