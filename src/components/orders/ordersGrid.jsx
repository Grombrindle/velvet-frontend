...
const OrdersGrid = () => {
  ...
  return (
    <div className="container4 mx-auto lg:mt-4 mt-28">
      <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6">
        {orders?.map((order) => (
          <div
            key={order.id}
            style={{ boxShadow: "0px 0px 28px 0px #00000040" }}
            className="w-full h-104 border border-[#959595] shadow-lg p-4 flex flex-col bg-white"
          >
            <div className="space-y-4 flex-1 flex flex-col">
              ...
              <Link href={`${localePrefix}/dashboard/orders/${order.id}`}>
                <button className="w-full h-12 cursor-pointer bg-black text-white text-sm font-bold">
                  {t("ORDER_DETAILS")}
                </button>
              </Link>
            </div>
